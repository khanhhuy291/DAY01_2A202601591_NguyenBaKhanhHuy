...
def compare_models(prompt: str) -> dict:
    ...
    """
   # 1. Gọi call_openai (GPT-4o)
    gpt4o_response, gpt4o_latency = call_openai(prompt)
    
    # 2. Gọi call_openai_mini (GPT-4o-mini)
    mini_response, mini_latency = call_openai_mini(prompt)
    
    # 3. Tính chi phí ước lượng cho GPT-4o
    # Giả định hằng số PRICING_PER_1K_TOKENS đã được khai báo ở đầu file
    ...
    # 4. Trả về dict kết quả với các key đúng như yêu cầu
    return {
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Dự phòng khi mất kết nối/model lạ/không dùng được tiktoken
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    """
    # 1. Đếm số token cho prompt và response
    input_tokens = count_tokens(prompt, model=model)
    output_tokens = count_tokens(response, model=model)

    # 2. Lấy bảng giá tương ứng với model (fallback về gpt-4o nếu không tìm thấy)
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])

    # 3. Tính chi phí input, output và tổng chi phí
    input_cost = (input_tokens / 1000) * pricing["input"]
    output_cost = (output_tokens / 1000) * pricing["output"]
    total_cost = input_cost + output_cost

    # 4. Trả về dictionary kết quả với đúng 5 key yêu cầu
    return {
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        reply_parts = []
        for chunk in stream:
            # Lấy content từ delta (fallback về chuỗi rỗng)
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply_parts.append(delta)

        print()  # Xuống dòng sau khi stream xong toàn bộ câu trả lời

        # Ghép phản hồi và cập nhật lịch sử hội thoại
        full_reply = "".join(reply_parts)
        history.append({"role": "assistant", "content": full_reply})

        # Giữ lại tối đa 3 lượt hội thoại gần nhất (6 messages)
        history = history[-6:]
...
def retry_with_backoff(
    ...
    # Vòng lặp bao gồm 1 lần gọi đầu tiên + max_retries lần thử lại
    for attempt in range(max_retries + 1):
        try:
            return fn()
        except Exception:
            # Nếu đã dùng hết lượt retry, ném lại lỗi cuối cùng
            if attempt == max_retries:
                raise
            # Tăng thời gian chờ theo lũy thừa 2 (exponential backoff)
...
def run_assistant(
    ...
    # Thiết lập hàm đọc input dự phòng nếu không được truyền vào
    if get_input is None:
        get_input = input
    ...
    while True:
        # 1. Kiểm tra giới hạn số lượt (max_turns)
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # Bỏ qua nếu người dùng chỉ gõ phím Enter (chuỗi rỗng)
        if not user_msg.strip():
            continue

        # 3. Chuẩn bị payload (messages) bao gồm: persona + history + user_msg
        messages = [{"role": "system", "content": persona}] + history + [{"role": "user", "content": user_msg}]

        # Đóng gói hàm gọi API (stream) thành tham số để truyền vào retry_with_backoff
        def api_call():
        ...
        # 4. Gọi API với cơ chế retry chịu lỗi
        stream = retry_with_backoff(api_call)

        # 5. Xử lý Streaming Output
        reply_parts = []
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply_parts.append(delta)
        print()  # Xuống dòng khi assistant trả lời xong

        reply = "".join(reply_parts)

        # 6. Cập nhật lịch sử (User + Assistant) và giới hạn ở 6 messages (3 lượt)
        ...
        # Quy đổi toàn bộ cục messages thành string để đếm token đầu vào gần đúng nhất
        prompt_str = str(messages)
        cost_metrics = estimate_cost(prompt_str, reply, model=OPENAI_MODEL)
        
        total_tokens += (cost_metrics["input_tokens"] + cost_metrics["output_tokens"])
        total_cost += cost_metrics["total_cost"]

    # 8. Trả về Dictionary tổng kết
    return {
        "num_turns": num_turns,
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        # Gọi hàm compare_models từ Part 1
        comparison = compare_models(prompt)
        # Thêm key 'prompt' như yêu cầu
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        # Cắt bớt văn bản quá dài để bảng không bị vỡ (max 40 char)
        def truncate(text: str) -> str:
            clean_text = str(text).replace("\n", " ").strip()
            return clean_text[:37] + "..." if len(clean_text) > 40 else clean_text
        
        prompt_text = truncate(r.get("prompt", ""))
        gpt4o_text = truncate(r.get("gpt4o_response", ""))
        mini_text = truncate(r.get("mini_response", ""))
        gpt4o_lat = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        mini_lat = f"{r.get('mini_latency', 0.0):.2f}s"
        
        rows += f"| {prompt_text} | {gpt4o_text} | {mini_text} | {gpt4o_lat} | {mini_lat} |\n"
